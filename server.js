...
// ====================================================================
// Legacy provider symbols are retained only as defensive stubs so old internal
// route names fail gracefully. Active music routes are handled by music-providers.js.
async function legacyProviderRemoved() {
  return { body: { code: 410, message: 'LEGACY_CHINESE_PROVIDER_REMOVED' } };
}
const search = legacyProviderRemoved;
const cloudsearch = legacyProviderRemoved;
const song_detail = legacyProviderRemoved;
const song_url = legacyProviderRemoved;
const song_url_v1 = legacyProviderRemoved;
const login_qr_key = legacyProviderRemoved;
const login_qr_create = legacyProviderRemoved;
const login_qr_check = legacyProviderRemoved;
const login_status = legacyProviderRemoved;
const logout = legacyProviderRemoved;
const user_account = legacyProviderRemoved;
const user_playlist = legacyProviderRemoved;
const comment_music = legacyProviderRemoved;
const artist_detail = legacyProviderRemoved;
const artist_top_song = legacyProviderRemoved;
const artist_songs = legacyProviderRemoved;
const like_song = legacyProviderRemoved;
const likelist = legacyProviderRemoved;
const song_like_check = legacyProviderRemoved;
const playlist_tracks = legacyProviderRemoved;
const playlist_track_add = legacyProviderRemoved;
const playlist_create = legacyProviderRemoved;
const playlist_detail = legacyProviderRemoved;
const playlist_track_all = legacyProviderRemoved;
const personalized = legacyProviderRemoved;
const recommend_resource = legacyProviderRemoved;
const recommend_songs = legacyProviderRemoved;
const dj_detail = legacyProviderRemoved;
const dj_program = legacyProviderRemoved;
const dj_hot = legacyProviderRemoved;
const dj_sublist = legacyProviderRemoved;
const user_audio = legacyProviderRemoved;
const dj_paygift = legacyProviderRemoved;
const record_recent_voice = legacyProviderRemoved;
const sati_resource_sub_list = legacyProviderRemoved;
const lyric = legacyProviderRemoved;
const lyric_new = legacyProviderRemoved;
...
const spotifyHostState = {
  lastSeen: 0,
  ready: false,
  connected: false,
  deviceId: '',
  deviceName: 'ShinaYuu Music',
  currentUri: '',
  currentTrackId: '',
  positionMs: 0,
  durationMs: 0,
...
function updateSpotifyHostState(payload = {}) {
  ...
  if (payload.ready != null) spotifyHostState.ready = !!payload.ready;
  if (payload.connected != null) spotifyHostState.connected = !!payload.connected;
  if (payload.deviceId != null) spotifyHostState.deviceId = String(payload.deviceId || '');
  if (payload.deviceName != null) spotifyHostState.deviceName = String(payload.deviceName || 'ShinaYuu Music');
  if (payload.currentUri != null) spotifyHostState.currentUri = String(payload.currentUri || '');
  if (payload.currentTrackId != null) spotifyHostState.currentTrackId = String(payload.currentTrackId || '');
  if (payload.positionMs != null) spotifyHostState.positionMs = Math.max(0, Number(payload.positionMs) || 0);
  if (payload.durationMs != null) spotifyHostState.durationMs = Math.max(0, Number(payload.durationMs) || 0);
  if (payload.isPlaying != null) spotifyHostState.isPlaying = !!payload.isPlaying;
...
function publicSpotifyHostState() {
  const alive = Date.now() - Number(spotifyHostState.lastSeen || 0) < 10000;
  return {
    ok: true,
    alive,
    ready: alive && !!spotifyHostState.ready,
    connected: alive && !!spotifyHostState.connected,
    deviceId: alive ? spotifyHostState.deviceId : '',
    deviceName: spotifyHostState.deviceName || 'ShinaYuu Music',
    currentUri: spotifyHostState.currentUri,
    currentTrackId: spotifyHostState.currentTrackId,
    positionMs: spotifyHostState.positionMs,
    durationMs: spotifyHostState.durationMs,
    isPlaying: alive && !!spotifyHostState.isPlaying,
...
function normalizeManifestUpdateInfo(data) {
  ...
  const patchInfo = patch && patch.downloadUrl ? {
    name: patch.name || updateAssetNameFromUrl(patch.downloadUrl) || `ShinaYuu-Music-${APP_VERSION}→${latestVersion}.patch.json`,
  ...
  const assetInfo = downloadUrl ? {
    name: asset.name || updateAssetNameFromUrl(downloadUrl) || `ShinaYuu-Music-${latestVersion}-Setup.exe`,
  ...
  return {
    ...
    release: {
      ...
      name: release.name || data.name || ('ShinaYuu Music v' + latestVersion),
...
async function readUpdateManifest(ref) {
  ...
  if (/^https?:\/\//i.test(value)) {
    const resp = await fetch(value, {
      headers: { 'User-Agent': `ShinaYuuMusic/${APP_VERSION}` },
    });
...
function localUpdateFallback(reason, opts) {
  ...
  return {
    ...
    release: {
      ...
      name: 'ShinaYuu Music v' + APP_VERSION,
      version: APP_VERSION,
      htmlUrl: '',
      downloadUrl: '',
...
function parseLatestYmlUpdateInfo(text, reason) {
  ...
  return {
    ...
    release: {
      ...
      name: 'ShinaYuu Music v' + latestVersion,
      version: latestVersion,
...
async function fetchLatestUpdateInfo() {
  ...
  try {
    const resp = await fetch(apiUrl, {
      ...
      headers: {
        'User-Agent': `ShinaYuuMusic/${APP_VERSION}`,
    ...
    return {
      ...
      release: {
        ...
        name: data.name || ('ShinaYuu Music v' + latestVersion),
        version: latestVersion,
...
function safeUpdateFileName(name, version) {
  const raw = String(name || '').trim() || `ShinaYuu-Music-${version || APP_VERSION}.exe`;
  const cleaned = raw
    .replace(/[<>:"/\\|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, ' ')
    .trim()
    .slice(0, 160);
  return cleaned || `ShinaYuu-Music-${version || APP_VERSION}.exe`;
}
...
async function handleSearch(keywords, limit) {
  console.log('[Search:Spotify+YouTube]', keywords, 'limit:', limit);
  const target = Math.max(1, Math.min(50, Number(limit) || 18));
  const settled = await Promise.allSettled([
    musicProviders.spotifySearch(keywords, target),
    musicProviders.youtubeSearch(keywords, target),
  ]);
  const spotifySongs = settled[0].status === 'fulfilled' ? settled[0].value : [];
  const youtubeSongs = settled[1].status === 'fulfilled' ? settled[1].value : [];
  const seen = new Set();
  return spotifySongs.concat(youtubeSongs).filter(song => {
    const key = String(song.name || '').toLowerCase() + '|' + String(song.artist || '').toLowerCase();
...
function buildWeatherMood(weather, date) {
  ...
  if (isStorm) {
    ...
    mood = {
      ...
      focus: 0.64,
      melancholy: 0.66,
      keywords: ['rainy day R&B', 'lofi rainy', 'Vietnamese slow songs', 'dream pop', 'late night rain playlist'],
    };
    ...
    mood = {
      ...
      focus: 0.72,
      melancholy: 0.54,
      keywords: ['winter acoustic', 'ambient piano', 'Japanese winter songs', 'indie folk', 'quiet playlist'],
    };
    ...
    mood = {
      ...
      focus: 0.46,
      melancholy: 0.30,
      keywords: ['summer chill', 'bossa nova', 'summer city pop', 'soft electronic', 'beach playlist'],
    };
  } else if (isCloud) {
    mood = {
      key: 'cloudy',
      ...
      focus: 0.58,
      melancholy: 0.52,
      keywords: ['cloudy day pop', 'indie rock mellow', 'neo soul', 'chillhop', 'acoustic indie'],
    };
  }
  ...
  if (isNight) {
    ...
    mood.keywords = ['late night R&B', 'late night jazz', 'ambient', 'lofi sleep', 'night drive playlist'].concat(mood.keywords.slice(0, 3));
  } else if (isMorning) {
    mood.title = mood.key.startsWith('rain') ? '雨晨电台' : '早晨电台';
    mood.energy = Math.max(mood.energy, 0.52);
    mood.keywords = ['morning commute', 'morning acoustic', 'sunrise indie', 'Vietnamese morning pop'].concat(mood.keywords.slice(0, 3));
  } else if (isDusk) {
    mood.title = mood.key.startsWith('rain') ? '黄昏雨声' : '黄昏电台';
    mood.melancholy = Math.max(mood.melancholy, 0.48);
    mood.keywords = ['sunset city pop', 'sunset playlist', 'indie city pop', 'soul pop'].concat(mood.keywords.slice(0, 3));
  }

  if (wind >= 28) {
    mood.energy = Math.max(mood.energy, 0.56);
    mood.keywords = ['road trip rock', 'windy day playlist'].concat(mood.keywords.slice(0, 4));
  }
...
function weatherRadioSeedQueries(mood) {
  ...
  if (key.includes('rain') || key.includes('storm')) return ['rainy day R&B', 'Vietnamese acoustic rain', 'late night soul', 'soft indie pop', 'rainy lofi'];
  if (key.includes('snow') || key.includes('cloudy')) return ['winter acoustic', 'cloudy day indie', 'ambient piano', 'soft folk', 'mellow alternative'];
  if (key.includes('humid')) return ['summer city pop', 'beach chill', 'bossa nova summer', 'tropical indie pop', 'sunset drive'];
  if (key.includes('night')) return ['late night R&B', 'Frank Ocean Pink + White', "Norah Jones Don't Know Why", 'night jazz', 'midnight soul'];
  return ['Vietnamese pop hits', 'sunny day playlist', 'soft pop', 'acoustic favorites', 'city pop'];
}
...
function fallbackWeatherForRadio(params, err) {
  ...
  return {
    ...
    mood: {
      ...
      focus: 0.55,
      melancholy: 0.35,
      keywords: ['Vietnamese pop', 'indie pop', 'city pop', 'upbeat playlist', 'chill pop'],
    },
  };
}
...
function scoreWeatherSong(song, mood) {
  ...
  if (/official|topic|records|music/.test(text)) score += 3;
  const key = String(mood && mood.key || '');
  if (key.includes('rain') && /rain|night|slow|r&b|soul|lofi/.test(text)) score += 5;
  if (key.includes('humid') && /summer|beach|city|pop|bossa|tropical/.test(text)) score += 5;
  if (key.includes('night') && /night|moon|jazz|soul|r&b|midnight/.test(text)) score += 5;
  if (key.includes('cloudy') && /cloud|folk|indie|mellow|acoustic/.test(text)) score += 5;
  return score;
}
...
async function handleModernMusicRoute(req, res, url, pn) {
  ...
  if (pn === '/api/youtube/engine/status') {
    try {
      const engine = await musicProviders.prepareYouTubeEngine();
      sendJSON(res, { ok: true, ...engine });
    } catch (error) {
      sendJSON(res, { ok: false, ...musicProviders.youtubeEngineStatus(), message: error.message || 'YouTube playback engine is unavailable.' }, 503);
    }
    return true;
  }
  ...
  if (pn === '/api/spotify/callback') {
    try {
      const query = Object.fromEntries(url.searchParams.entries());
      const result = await musicProviders.completeSpotifyLogin(query);
      sendHtml(res, musicProviders.spotifyCallbackHtml(true, result && result.complete ? '' : 'profile_pending'));
    } catch (error) {
      sendHtml(res, musicProviders.spotifyCallbackHtml(false, error.message), Number(error.status) || 400);
    }
    return true;
  }

  if (pn === '/api/spotify/login/result') {
    try {
  ...
  if (pn === '/api/spotify/host/event' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      updateSpotifyHostState(body || {});
      if (body && body.type === 'ready') {
        console.log(`[SpotifyHost] ready device=${String(body.deviceId || '-')}`);
      } else if (body && body.type === 'error' && body.error) {
        console.warn(`[SpotifyHost] ${String(body.errorType || 'error')}: ${String(body.error)}`);
      }
  ...
  if (pn === '/api/spotify/player/token') {
    try { sendJSON(res, { ok: true, ...(await musicProviders.spotifyPlayerToken()) }); }
    catch (error) { modernProviderError(res, error, 401); }
    return true;
  }
  ...
  if (pn === '/api/spotify/audio-analysis') {
    try {
      ...
      sendJSON(res, { ok: true, ...(await musicProviders.spotifyAudioAnalysis(id)) });
    } catch (error) {
      // Some Spotify applications do not have access to audio-analysis. The
      // player handles 403/404 by using a position-locked visual fallback.
      modernProviderError(res, error, Number(error.status) || 502);
    }
    return true;
  }

  if (pn === '/api/spotify/player/devices') {
    try { sendJSON(res, { ok: true, devices: await musicProviders.spotifyDevices() }); }
    catch (error) { modernProviderError(res, error, 500); }
    return true;
  }

  if (pn === '/api/spotify/player/state') {
    try { sendJSON(res, { ok: true, ...(await musicProviders.spotifyPlaybackState()) }); }
    catch (error) { modernProviderError(res, error, 500); }
    return true;
  }

  if (pn === '/api/spotify/player/transfer' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      await musicProviders.spotifyTransferPlayback(String(body.deviceId || ''), !!body.play);
      sendJSON(res, { ok: true });
    } catch (error) { modernProviderError(res, error, 500); }
    return true;
  }

  if (pn === '/api/spotify/player/play' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
  ...
  if (pn === '/api/spotify/player/pause' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      await musicProviders.spotifyPausePlayback(String(body.deviceId || ''));
      sendJSON(res, { ok: true });
    } catch (error) { modernProviderError(res, error, 500); }
    return true;
  }

  if (pn === '/api/spotify/player/resume' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      await musicProviders.spotifyResumePlayback(String(body.deviceId || ''));
      sendJSON(res, { ok: true });
    } catch (error) { modernProviderError(res, error, 500); }
    return true;
  }
  ...
  if (pn === '/api/spotify/player/volume' && req.method === 'POST') {
    try {
      const body = await readRequestBody(req);
      await musicProviders.spotifySetPlaybackVolume(Number(body.volumePercent || 0), String(body.deviceId || ''));
      sendJSON(res, { ok: true });
    } catch (error) { modernProviderError(res, error, 500); }
    return true;
  }
  ...
  if (pn === '/api/login/qr/key' || pn === '/api/login/qr/create' || pn === '/api/login/qr/check') {
    sendJSON(res, { error: 'SPOTIFY_USES_BROWSER_OAUTH', code: 800 }, 400);
    return true;
  }
  ...
  if (pn === '/api/podcast/search' || pn === '/api/podcast/hot') {
    try {
      const keywords = pn.endsWith('/hot') ? '' : (url.searchParams.get('keywords') || '');
      const limit = Math.max(6, Math.min(30, parseInt(url.searchParams.get('limit') || '18', 10) || 18));
      const podcasts = await musicProviders.youtubePodcastSearch(keywords, limit);
      sendJSON(res, { provider: 'youtube', podcasts, total: podcasts.length, more: false });
    } catch (error) {
      console.error('[YouTubePodcast]', error);
      sendJSON(res, { provider: 'youtube', error: error.message, podcasts: [] }, Number(error.status) || 500);
    }
    return true;
  }

  if (pn === '/api/podcast/detail' || pn === '/api/podcast/programs') {
    try {
      const id = url.searchParams.get('id') || url.searchParams.get('rid') || '';
      const data = await musicProviders.youtubePodcastPrograms(id);
      if (pn.endsWith('/detail')) sendJSON(res, { provider: 'youtube', podcast: data.radio });
      else sendJSON(res, { provider: 'youtube', radio: data.radio, programs: data.programs, total: data.programs.length, more: false });
    } catch (error) {
      console.error('[YouTubePodcastPrograms]', error);
      sendJSON(res, { provider: 'youtube', error: error.message, programs: [] }, Number(error.status) || 500);
    }
    return true;
  }

  if (pn === '/api/podcast/my') {
    sendJSON(res, { loggedIn: false, provider: 'youtube', collections: [] });
    return true;
  }

  if (pn === '/api/podcast/my/items') {
    sendJSON(res, { loggedIn: false, provider: 'youtube', items: [] });
    return true;
  }
  ...
  if (pn === '/api/search') {
    try {
      ...
      const songs = keywords ? await musicProviders.spotifySearch(keywords, limit) : [];
      sendJSON(res, { provider: 'spotify', songs, result: songs, configured: musicProviders.providerConfig().spotifyClientId !== '' });
    } catch (error) {
      console.error('[SpotifySearch]', error);
      sendJSON(res, { provider: 'spotify', error: error.message, songs: [], result: [] }, Number(error.status) || 500);
    }
    return true;
  }
  ...
  if (pn === '/api/song/url') {
    try {
      ...
      const data = await musicProviders.resolveSpotifyPlayback(id, quality);
      sendJSON(res, {
        ...data,
        loggedIn: status.loggedIn,
        vipType: status.vipType || 0,
        vipLevel: status.vipLevel || 'free',
        isVip: !!status.isVip,
        isSvip: false,
        vipLabel: status.vipLabel || 'Spotify',
      });
    } catch (error) {
      console.error('[SpotifyPlayback]', error);
      sendJSON(res, {
        url: null,
        proxyUrl: null,
        playable: false,
        ...
        transport: 'spotify',
        reason: Number(error.status) === 401 ? 'login_required' : 'spotify_playback_unavailable',
        message: error.message || 'Spotify playback is unavailable.',
        restriction: { provider: 'spotify', category: Number(error.status) === 401 ? 'login_required' : 'playback_unavailable', message: error.message || 'Spotify playback is unavailable.', action: Number(error.status) === 401 ? 'login' : 'retry' },
  ...
  if (pn === '/api/lyric' || pn === '/api/qq/lyric') {
    try {
      ...
      console.error('[LRCLIB]', error);
      sendJSON(res, { lyric: '', tlyric: '', yrc: '', plainLyric: '', source: 'lrclib', error: error.message });
    }
    return true;
  }
  ...
  if (pn === '/api/song/like/check') {
    try {
      ...
      const liked = {};
      ids.forEach((id, index) => { liked[id] = !!checks[index]; });
      sendJSON(res, { loggedIn: true, liked, ids: ids.filter((id) => liked[id]) });
    } catch (error) { modernProviderError(res, error); }
    return true;
  }
  ...
  if (pn === '/api/artist/detail') {
    try {
      const limit = Math.max(1, Math.min(80, parseInt(url.searchParams.get('limit') || '36', 10) || 36));
      sendJSON(res, await musicProviders.spotifyArtistDetail(url.searchParams.get('id') || '', limit));
    } catch (error) { modernProviderError(res, error); }
    return true;
  }
  ...
  if (pn === '/api/qq/artist/detail') {
    try {
      const limit = Math.max(1, Math.min(80, parseInt(url.searchParams.get('limit') || '36', 10) || 36));
      sendJSON(res, await musicProviders.youtubeArtistDetail(url.searchParams.get('mid') || url.searchParams.get('id') || '', limit));
    } catch (error) { modernProviderError(res, error); }
    return true;
  }

  if (pn === '/api/song/comments' || pn === '/api/qq/song/comments') {
    try {
      const youtube = pn.includes('/qq/');
      let id = youtube ? (url.searchParams.get('mid') || url.searchParams.get('id') || '') : '';
      if (!youtube) {
        const metadata = musicProviders.songMetadata(url.searchParams.get('id') || '', 'spotify');
        if (metadata && metadata.youtubeId) id = metadata.youtubeId;
      }
      const comments = id ? await musicProviders.youtubeComments(id, parseInt(url.searchParams.get('limit') || '20', 10) || 20) : [];
      sendJSON(res, { id, total: comments.length, comments, hot: false });
    } catch (error) { sendJSON(res, { comments: [], error: error.message }); }
    return true;
  }

  if (pn === '/api/discover/home') {
    try { sendJSON(res, await musicProviders.discoverHome()); }
    catch (error) { modernProviderError(res, error); }
    return true;
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (await handleModernMusicRoute(req, res, url, pn)) return;

  if (pn === '/api/app/version') {
    sendJSON(res, {
      name: APP_PACKAGE.name || 'shinayuu-music',
      productName: APP_PACKAGE.productName || 'ShinaYuu Music',
      version: APP_VERSION,
      update: {
  ...
  if (pn === '/api/cover') {
    try {
      ...
      const coverHeaders = { 'User-Agent': UA };
      try {
        const host = new URL(coverUrl).hostname.toLowerCase();
        if (host.includes('spotifycdn') || host.includes('scdn.co')) coverHeaders.Referer = 'https://open.spotify.com/';
        else if (host.includes('ytimg.com') || host.includes('youtube.com')) coverHeaders.Referer = 'https://www.youtube.com/';
      } catch (_) {}
  ...
  if (pn === '/api/audio') {
    try {
      ...
      const audioUrl = descriptor && descriptor.url || url.searchParams.get('url');
      if (!audioUrl) {
        res.writeHead(streamToken ? 410 : 400, { 'Content-Type': 'text/plain; charset=utf-8' });
        res.end(streamToken ? 'Audio stream expired. Please retry the track.' : 'Missing url');
        return;
      }
      const range = req.headers.range || '';
      const hdr = {
        ...audioProxyHeadersFor(audioUrl, range),
        ...((descriptor && descriptor.headers) || {}),
      };
      // The browser Range header must win over stale headers returned by yt-dlp.
      ...
      if (!up.ok && up.status !== 206) {
        ...
        res.end(detail.slice(0, 1000) || `Upstream audio HTTP ${up.status}`);
        return;
      }
...
server.listen(PORT, HOST, () => {
  console.log('======================================================');
  console.log('[ShinaYuu Music] http://127.0.0.1:' + PORT);
  console.log('[Spotify] OAuth PKCE | [YouTube] yt-dlp playback engine');
  console.log('======================================================');
  musicProviders.prepareYouTubeEngine()
    .then((status) => console.log('[YouTubeEngine] Ready:', status.version || status.engine))
    .catch((error) => console.warn('[YouTubeEngine] Preparation deferred:', error.message));
});
