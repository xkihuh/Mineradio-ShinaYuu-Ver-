...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for ShinaYuu Music icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'ShinaYuuMusic';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`ShinaYuu Music executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`ShinaYuu Music icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting ShinaYuu Music resources  rcedit=${rceditPath}`);
  execFileSync(rceditPath, [
    exePath,
    '--set-icon', iconPath,
    '--set-version-string', 'FileDescription', 'ShinaYuu Music',
    '--set-version-string', 'ProductName', 'ShinaYuu Music',
    '--set-version-string', 'CompanyName', 'ShinaYuu',
